...
// Tipos fuertes para el provider
...
function makeClient() {
  ...
  // Fallback (no gratis normalmente)
...
export class OpenAiPlannerProvider {
  ...
  /**
   * Extrae un array JSON de tareas desde un brief en lenguaje natural.
   * Devuelve `unknown` (luego AiService valida con Zod).
   */
  async extractTasks({
    brief,
    timezone,
...
Timezone: ${timezone}. Hoy: ${todayISO}. Duración por defecto: ${String(
...
Devuelve EXCLUSIVAMENTE un array JSON válido (sin comentarios, sin texto extra).`;
    ...
    // ✅ Usar Chat Completions (compatible con Groq/Ollama)
    ...
    // Parse seguro de la respuesta (content puede ser string o array)
    ...
    try {
      ...
      // si no vino JSON válido, devolvemos []
      return [];
    }
  }
}
